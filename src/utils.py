...
def linearise_input(data_row, method, max_fts=15, data_only=False):
    ...
    other_classes = "& ".join(
        [f"{k} {v}" for k, v in classes_dict.items() if k != chosen_class]
    )
    ...
    negl_fts = "& ".join(
        [f"{a} " for a, b in zip(data_row["feature_nums"], sign) if b == "null"]
    )
    negl_fts = "None" if negl_fts == "" else negl_fts

    essel_input = (
        f"| predicted class | {chosen_class} {classes_dict[chosen_class]} | other classes | {other_classes} "
    ...
    ord_first_fts = " ".join(
        [
            f"| {o} | {f} {s} {v}"
            for o, f, s, v in zip(ordinals, feature_nums, sign, values)
        ]
    )
    ft_first_fts = " ".join(
        [
    ...
    text_input = (
        f"Predicted class is {chosen_class}, value of {classes_dict[chosen_class]}. "
    ...
    if data_only:
        ...
        preamble = " | Questions | "
        questions = " ".join(
            [f"{idx+1}. {q}" for idx, q in enumerate(data_row["narrative_questions"])]
        )
    ...
    if method == "essel":
        ...
        data_row["input"] = (
            f"Predicted class is {chosen_class}, value of {classes_dict[chosen_class]}. "
            f"Other classes and values are {other_classes}. "
            f"Top features are {commas_and_and(feature_nums[:3])}. "
        ...
        raise ValueError(
            "method must be one of essel, ord_first or ft_first, text or baseline_input."
        )
...
def form_stepwise_input(data_row, method, max_fts):
    ...
    preamble = "\n <br> <br> Using the above information, answer the following \
            in detail: <br> <br> "
    data_row["input"] = [
        data_row["input"] + preamble + q for q in data_row["narrative_questions"]
    ]
    return data_row
...
def convert_to_features(batch, tokenizer, max_input_length=400, max_output_length=350):
    if type(batch["input"][0]) == list:
        input_encodings = [
            tokenizer(
                i, padding="max_length", truncation=True, max_length=max_input_length
            )
            for i in batch["input"]
        ]
...
def simplify_feat_names(row):
    ...
    """
    row["narrative_questions"] = [
        re.sub(r" \(([^()]*\bequal[^()]*)\)", "", q) for q in row["narrative_questions"]
    ]
    return row
...
def label_qs(row):
    ...
    if (
        ...
        narr_qs[0] == "Summarize the prediction for the given test example?"
        and narr_qs[1]
        ...
        narr_qs[0]
        == "Summarize the prediction made for the test under consideration along with the likelihood of the different possible class labels."
        and narr_qs[1][:53] == "Summarize the direction of influence of the variables"
    ):
        label = "H"
    else:
...
def simplify_narr_question(row):
    ...
    if label in ["A", "B"]:
        ...
        q4 = (
            f"Summarise these negligible features ({commas_and_and(mentioned_fts[3])})."
        )
...
def nums_to_names(narration, class2name_dict, ft_num2name_dict):
    ...
    cls_ptn = re.compile(
        "|".join([f"{k}\\b" for k in class2name_dict.keys() if k != None])
    )
...
def old_linearise_input(data_row, method, max_fts=15, data_only=False):
    ...
    fts = "&& ".join([f"{a} " for a in feature_nums])
    pos_fts = "&& ".join(
        [f"{a} " for a, b in zip(feature_nums, sign) if b == "positive"]
    )
    nega_fts = "&& ".join(
        [f"{a} " for a, b in zip(feature_nums, sign) if b == "negative"]
    )
    negl_fts = "&& ".join(
        [f"{a} " for a, b in zip(feature_nums, sign) if b == "negligible"]
    )
    negl_fts = "None" if negl_fts == "" else negl_fts
