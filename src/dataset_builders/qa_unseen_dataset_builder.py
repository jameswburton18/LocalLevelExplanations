...
DESCRIPTION = """\
    This is a collection of 469 question answer pairs. This is designed to be
    used as a test set to assess the performance of the QA model on unseen
    data.  
    """
...
class QADatasetBuilder(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        datasets.BuilderConfig(
            name="text-exp-qa-unseen",
            version=VERSION,
            description="Question Answering Hard Dataset based on textual explanations",
        ),
    ]
...
if __name__ == "__main__":
    ...
    dataset = datasets.load_dataset(
        "src/dataset_builders/qa_unseen_dataset_builder.py",
        download_mode="force_redownload",
    )  # , ignore_verifications=True)

    # Save the dataset to huggingface
    dataset.push_to_hub("text-exp-qa-unseen", private=True)
