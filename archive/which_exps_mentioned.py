...
def find_mentioned_fts(data_row):
    ...
    data_row["labels"] = torch.tensor(
        [1.0 if option in data_row["mentioned_fts"] else 0.0 for option in options]
    )
    return data_row


def convert_to_features_clf(
    batch, tokenizer, max_input_length=400, max_output_length=350
):
...
def main():
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        args["model_base"], num_labels=50, problem_type="multi_label_classification"
    )
